...
import (
	...
	cpfi "go.etcd.io/raft/v3/rafi.client"
	pb "go.etcd.io/raft/v3/raftpb"
	"go.etcd.io/raft/v3/tracker"
)

// cpfi begin
func BeforeSendReq(m *pb.Message) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgApp,
		Term: m.Term,
	}
...
func AfterSendReq(m *pb.Message) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgApp,
		Term: m.Term,
	}
...
func BeforeSendAck(m *pb.Message) bool {
	msg := &cpfi.CPFI_msg{
...
func AfterSendAck(m *pb.Message) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgAppResp,
		Term: m.Term,
	}
...
func BeforeRecvAck(m *pb.Message) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgAppResp,
		Term: m.Term,
	}
...
func BeforeRecvReq(m *pb.Message) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgApp,
		Term: m.Term,
	}
...
func (r *raft) send(m pb.Message) {
	...
	// cpfi begin
	if m.Type == pb.MsgApp {
		if ok := BeforeSendReq(&m); !ok {
			return
		}
	}

	if m.Type == pb.MsgAppResp {
		if ok := BeforeSendAck(&m); !ok {
			return
		}
	}
	// cpfi end
	...
	// cpfi begin
	if m.Type == pb.MsgApp {
		if ok := AfterSendReq(&m); !ok {
			return
		}
	}

	if m.Type == pb.MsgAppResp {
		if ok := AfterSendAck(&m); !ok {
			return
		}
	}
	// cpfi end
