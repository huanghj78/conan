...
import (
	...
	cpfi "github.com/hashicorp/raft/rafi.client"
)

// cpfi begin
func BeforeSendReq(m *AppendEntriesRequest) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgApp,
		Term: m.Term,
	}
...
func AfterSendReq(m *AppendEntriesRequest) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgApp,
		Term: m.Term,
	}
...
func BeforeSendAck(m *AppendEntriesResponse) bool {
	msg := &cpfi.CPFI_msg{
...
func AfterSendAck(m *AppendEntriesResponse) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgAppResp,
		Term: m.Term,
	}
...
func BeforeRecvAck(m *AppendEntriesResponse) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgAppResp,
		Term: m.Term,
	}
...
func BeforeRecvReq(m *AppendEntriesRequest) bool {
	msg := &cpfi.CPFI_msg{
		Type: cpfi.MsgApp,
		Term: m.Term,
	}
...
// func (r *Raft) runCandidate() {
//     // r.currentTerm = MAX_UINT64
//     voteCh := r.electSelf() // increase r.currentTerm
//     // r.currentTerm = 0 due to overflow
//     for r.getState() == Candidate {
//         select {
//         case vote := <-voteCh:
//             // Check if the term is greater than ours, bail
...
//                 r.logger.Debug("newer term discovered, fallback to follower", "term", vote.Term)
//                 r.setState(Follower)
//                 // lead currentTerm to MAX_UINT64
//                 r.setCurrentTerm(vote.Term)
//                 return
//             }
//         case <-electionTimer:
//             r.mainThreadSaturation.working()
//             r.logger.Warn("Election timeout reached, restarting election")
//             // lead currentTerm to 0
//             return
//     }
// }

// for {
//     err = writeCommand(conn, command, timeout)
//     if err == nil {
//         break
//     }
//     nRetries++
